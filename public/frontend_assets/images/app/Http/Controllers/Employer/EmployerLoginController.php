...
class EmployerLoginController extends Controller
{
    ...
    public function loginView()
    {
        ...
        return view('employer.auth.login', compact('pageTitle'));
    }
        ...
        try {
            ...
            if (!$user) {
                return response()->json([
                    'status' => false,
                    'not_registered' => true,
                    'message' => 'Employer record not found. Please complete registration first.'
                ], 422);
            }
            ...
            $otp = $this->createOtp($request->phone, 'employer_login');
            ...
            session(['temp_employer_login_phone' => $request->phone]);
    ...
    public function verifyLoginOtp(Request $request)
    {
        try {
            ...
            $result = $this->validateOtp($request->phone, $request->otp, 'employer_login');
            ...
            session([
                'verified_employer_login_phone' => $request->phone,
                'employer_login_phone_locked_at' => now()->timestamp
            ]);

            // Remove temp phone
            session()->forget('temp_employer_login_phone');
    ...
    public function login(Request $request)
    {
        try {
            ...
            $verifiedPhone = session('verified_employer_login_phone');
            $lockedAt = session('employer_login_phone_locked_at');
            ...
            // Clear session data
            session()->forget(['verified_employer_login_phone', 'employer_login_phone_locked_at']);

            return response()->json([
                'status' => true,
                'message' => 'Login Successful',
                'redirect' => route('employer.myjobs')
            ]);
